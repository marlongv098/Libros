...
1. Conductores: 1200 $\lyxmathsym{€}$

2. Autobuses: 90,000 $\lyxmathsym{€}$

3. Combustible: 2 $\lyxmathsym{€}$ por litro 

4. Mantenimiento: 500 $\lyxmathsym{€}$ por revisión
...
– $x_{1}$: conductores (1200 €/mes c/u)

– $x_{2}$: autobuses (90 000 €/mes c/u)

– $x_{3}$: combustible (2 €/L)

– $x_{4}$: mantenimiento (500 € por revisión)

– $x_{5}$: limpieza profunda (200 € por servicio)
...
• Ruta 1 ($R_{1}$): cada autobús realiza 1 limpieza profunda mensual.

• Ruta 2 ($R_{2}$): por ser la ruta con mayor flujo de pasajeros, cada autobús recibe una limpieza adicional al mes (en promedio 0.5 más por autobús respecto de $R_{1}$).

• Ruta 3 ($R_{3}$): atraviesa zonas rurales y polvorientas, por lo que requiere 1.5 limpiezas adicionales respecto de $R_{1}$.

• Ruta 4 ($R_{4}$): al operar en zonas centrales con alta rotación de pasajeros, se realizan 2 limpiezas adicionales respecto de $R_{1}$.
...
1. En una plataforma de redes sociales, cinco influencers ($I_{1}$,$I_{2}$,$I_{3}$,$I_{4}$,$I_{5}$) interactúan y afectan las opiniones de sus seguidores con distinta intensidad.

a) Para modelar este comportamiento, se le asignaron pesos a cada conexión, donde:

1) Un peso alto indica mayor influencia sobre el otro influencer. 

2) Un peso bajo indica menor influencia.

b) La relación de influencia entre ellos se modela de la siguiente manera:
...
1) Representación de la Influencia: 

A. Construya la matriz de adyacencia ponderada $W$, donde $W_{ij}$ representa la influencia de $I_{i}$ sobre $I_{j}$ 

B. Dibuje el grafo dirigido con pesos para visualizar la estructura de influencia.

2) Análisis de Influencia Global: 

A. ¿Qué influencer tiene la mayor suma de pesos salientes (es decir, influye más en los demás)? 

B. ¿Qué influencer tiene la mayor suma de pesos entrantes (es decir, es más influenciado por otros)? 

3) Impacto de un Influencer Clave: 

A. Si $I_{1}$ deja de publicar contenido, ¿cómo afecta la influencia en la red?

B. Si $I_{5}$ aumenta en un $50\%$ su influencia sobre $I_{2}$ e $I_{3}$, ¿cómo cambia la propagación de la información en la red?

4) Propagación de una Opinión o Producto Nuevo: 

A. Supongamos que un nuevo producto es promocionado inicialmente solo por $I_{1}$. Modela la propagación del producto en la red considerando que cada influencer adopta el producto con una probabilidad proporcional a la influencia que recibe. 

B. ¿Cuánto tiempo (en términos de pasos de propagación) tarda en llegar a todos los influencers?
...
a) Inicio (I): Página principal. 

b) Productos (P): Catálogo de productos. 

c) Blog (B): Artículos informativos. 

d) Carrito (C): Página de compra. 

Se quiere modelar las transiciones entre estas páginas como una Cadena de Markov para:

a) Predecir la próxima sección que visitará un usuario dado su estado actual. 

b) Calcular el estado de equilibrio (distribución estacionaria) para saber la proporción de tiempo que los usuarios pasan en cada página a largo plazo.
